...
import { Grid, Card, CardContent, Box, Table, TableHead, TableRow, TableCell, TableBody } from "@mui/material";
                                    ...
                                    {monitorAlerts.length > 0 &&
                                        monitorAlerts.map((alert) => (
                                            <TableRow key={alert.id}>
                                                <TableCell>{moment(alert.timeStamp).format('MM/DD/YYYY HH:mm:ss')}</TableCell>
                                                <TableCell>{alert.monitorName}</TableCell>
                                                <TableCell>{alert.message}</TableCell>
                                                <TableCell>{alert.screenShotUrl != null ? (<a href={alert.screenShotUrl} style={{ textDecoration: "none", color: "unset" }}>Download</a>) : ("N/A")}</TableCell>
                                            </TableRow>
                                        ))}
                                </TableBody>
                            </Table>
